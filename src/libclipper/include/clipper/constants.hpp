...
enum RedisDBTable {
  ...
  REDIS_MODULE_ENDPOINT_LINKS_DB = 7,
  REDIS_METADATA_DB_NUM = 8,  // used to store Clipper configuration metadata (ex: model version metadata)
  REDIS_APPLICATION_DB_NUM = 9, // UNUSED - KEEPING (TEMPORARILY?) FOR COMPATIBILITY
  REDIS_APP_MODEL_LINKS_DB_NUM = 10 // UNUSED - KEEPING (TEMPORARILY?) FOR COMPATIBILITY
};
